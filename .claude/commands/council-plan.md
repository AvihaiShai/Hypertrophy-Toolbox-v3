...
1. **Draft Plan v1.** After any size-required Gate 0 sign-off, the read-only manager delegates Plan v1 drafting to `product-manager`, which writes Plan v1 at the canonical `docs/<feature>/PLANNING.md` path using the [docs/ai_workflow/PLAN_REVIEW_TEMPLATE.md](../../docs/ai_workflow/PLAN_REVIEW_TEMPLATE.md) shell. Existing council artifacts at other paths are legacy; do not relocate them merely to satisfy this rule. Include: goal, scope (in/out), artifact list with paths, effort, sequence, expected gates.
