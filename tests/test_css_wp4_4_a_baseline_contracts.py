...
EXPECTED_SNAPSHOT_COUNTS = {
    ...
    "linux/visual.spec.ts-snapshots": 60,
...
def test_fatigue_baselines_exist_on_windows_and_are_pending_on_linux() -> None:
    """N7 — creation on both platforms, and an honest record of the gap.

    The Windows set is generated locally. The Linux set can only be produced by
    the `visual-linux` deep-gate job, so until that lands the asymmetry is
    recorded here rather than left for a later packet to discover.
    """
    baseline = _baseline()
