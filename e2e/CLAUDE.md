...
- **Captures are segmented above 16,384 px.** Chromium truncates a taller `fullPage` screenshot to a flat, unpainted tail instead of failing. `expectFullPageScreenshot()` in `visual-helpers.ts` splits such a page into `<name>-segment-N.png` bands of `CAPTURE_SEGMENT_HEIGHT_PX`; every shorter page keeps its single baseline and its existing name. Only `user-profile-mobile-{dark,light}` is affected, so `visual.spec.ts` carries **68** baselines per platform (66 + 4 segments − 2 retired) while `visual-baseline-thumbnails.spec.ts` stays at 18. Re-baselining that pair must also bump `EXPECTED_SNAPSHOT_COUNTS` in `tests/test_css_wp4_4_a_baseline_contracts.py`, regenerate `snapshotManifest` in `docs/CSS_PHASE4_WP4_4_A_BASELINE.json` (`scripts/css_audit/emit_baseline.py`), and empty `AWAITING_SEGMENTED_REGENERATION` in `tests/test_visual_capture_contracts.py`.
