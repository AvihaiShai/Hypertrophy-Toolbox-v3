...
- **axe findings are registered, never suppressed.** `accessibility.spec.ts` owns `AXE_REGISTER`, which pins the *exact* node count of every WCAG violation the app produces per route per theme. It fails three ways — a new rule, a grown count, or a registered violation that stopped firing — so a fix must be recorded just as deliberately as a defect. Every rule it tolerates has a row in [`docs/testing_phase2/A11Y_EXCEPTIONS.md`](../docs/testing_phase2/A11Y_EXCEPTIONS.md), bound by `tests/test_axe_contracts.py` in the required pytest gate. Editing a count to clear a red is the one move that turns it back into a baseline.
...
| `visual` | **`visual-windows` (`ci.yml`) on every PR and every push to `main`/`develop`**, plus the `visual-linux` job in `deep-gate.yml` | Cross-OS rendering. `visual-windows` byte-compares the win32 corpus on `windows-2022`; `visual-linux` byte-compares the linux corpus on `ubuntu-24.04`. Neither is a **required** branch-protection context. See "Visual spec contract" below. |
| `visual-baseline-thumbnails` | `visual-linux` job only (weekly schedule + opt-in dispatch) | Deliberately excluded from `visual-windows`: #322 proved `plan-desktop-light-simple` races on `windows-2022`. Its **15** win32 captures therefore have no CI path at all and can stale silently. See "Visual spec contract" below. |
| `workout-plan-desktop-contract` | `visual-linux` job only (weekly schedule + opt-in dispatch) | Carries the coverage of the five byte-gate-exempt captures as computed style / geometry / DOM structure. It needs the plan-bearing visual seed, so it runs in `visual-linux` (which sets `PW_VISUAL_SEED=1`) rather than on the required functional path, and is excluded from the deep gate's "full suite (minus visual)" step for the same reason. |
...
- **Where they run.** *(Corrected 2026-08-29. This bullet read "Manual deep gate only … opt-in behind the `run_visual` input (`workflow_dispatch`-only)", which was false on two counts. The "never a **required** status check" half was and remains true.)*
  - `visual.spec.ts` runs in **`visual-windows` (`ci.yml`) on every pull request and every push to `main`/`develop`**, pinned to `windows-2022`, compare-only against `e2e/__screenshots__/win32/`. It is the 13th *expected* context, **not** one of the 12 required ones, so `main` can merge with it red.
  - **`visual-linux` also runs on the weekly `schedule`** (`cron: '17 3 * * 1'`), not only on dispatch. Its `if:` is `github.event_name == 'schedule' || inputs.run_visual`, and a `schedule` event pins `MODE=compare` regardless of the input. It runs `visual.spec.ts`, `visual-baseline-thumbnails.spec.ts` **and** `workout-plan-desktop-contract.spec.ts` together.
  - `visual-baseline-thumbnails.spec.ts` appears in **no** `ci.yml` job. Its **15** linux captures are compared weekly; its **15** win32 captures are compared only by a local run and can stale silently. Fixing the `windows-2022` race is the stated precondition for widening `visual-windows` to it (#322).
  - An `if:`-gated, non-required job cannot block merge — so a green PR is evidence about the **win32 `visual.spec.ts`** corpus only.
