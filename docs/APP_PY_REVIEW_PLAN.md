...
`deep-gate.yml`'s `frozen-windows` job remained the manual twin until Packet R2-b (2026-08-15)
folded it into the same reusable `_packaged-windows.yml` workflow, so the two are now one
definition with three callers rather than a twin pair.
