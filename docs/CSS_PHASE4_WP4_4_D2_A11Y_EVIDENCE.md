...
**The census oracle alone would have shipped C50 — this is the packet's most
important methodological result.** The full matrix reports C50 `REMOVABLE` both
before and after the edit, and it is not wrong to do so: its synthetic pass adds
...
Ownership-by-sentinel answers *"does this declaration win?"*; it does not
answer *"which declaration wins?"*, and only the second question exposes a
same-value owner transfer. **M6b: an annotation that is asymmetric with a
retained sibling requires a declaration-owner adjudication, not a census
verdict.**
...
| Red-path proof (`redpath_d2.py`) | **22/22** go red under their own violation; every source restored **sha256-identical**; green after |
| Full `pytest tests/` | **2,268 passed, 1 skipped** (431.27s) |
...
| `accessibility`, `dark-mode`, `smoke-navigation`, `summary-pages`, `volume-progress`, `fatigue`, `fatigue-stage4-smokes`, `ui-hardening`, `nav-dropdown` | **133 passed** (4.5m) |
| `visual.spec.ts`, Chromium, full matrix, `PW_VISUAL_SEED=1` | **65 passed, 1 failed** — the ledgered red, below |
| Same-CSS **pixel** control | identical **875 / 882 / 875** with `a11y.css` byte-identical to `main` |
| Positive flip check (`d2_flip.mjs`) | **PASS** |
| BEFORE matrix | **11,654 contexts**, same-CSS drift **0/466**, contamination **0** |
| AFTER matrix | **11,494 contexts**, same-CSS drift **0/459**, contamination **0**, subject stable |
| BEFORE→AFTER verdicts, all 51 | **exactly one change** — C34 `REMOVABLE` → dropped `priority-not-important` |
| Keyboard focus-indicator census | d2 inside `main`'s own run-to-run spread (§5.5) |
...
Test count reconciles: `main` @ `3a23333` collects **2,263**; this branch adds
the 6 new d2 contracts and collects **2,269** (2,268 passed + 1 skipped).

**The red-path proof was itself broken and is reported that way.** Its first run
reported `14/22`, with **8 cases silently no-op** — the sources on this checkout
are CRLF, and every mutation written with a plain `\n` matched nothing. A no-op
mutation produces a line of output while proving nothing, which is the same
class of failure as a control that cannot fail. The helper now tries both line
endings, a no-op is labelled *proof broken*, and the rerun is a genuine
**22/22**. The eight affected cases were `mixed_selector_lists`,
`focus_visible_premise` (×2), `no_custom_property`, `certified_removal_kept`,
`asymmetric_sibling`, `is-invalid:focus` retention, and `@-moz-document`.
...
| Control | Result |
|---|---|
| cascade controls CTLA, CTLB, CTLC, CTLC2, CTLD, CTLE | **6/6 pass** |
...
Packet `d` must not weaken keyboard focus visibility, so the census is reported
with a control rather than as a bare number. An element counts as having an
indicator when `outline-style ≠ none` with `outline-width > 0`, **or**
`box-shadow ≠ none`.
...
between the two matrices, so the two runs traversed different DOMs; the stop
counts differ for the same reason. Re-measured in one session against one
database:
...
`main` disagrees with itself across a spread of **3** (1,306–1,309); d2's 1,308
sits inside it, and the stop count is identical in all five runs. The four
differing contexts all resolve to the **same** element —
`html/body/header/div/nav/ul[0]/li[2]/a`, a navbar link, at `data-scale` 3 —
and the direction is inconsistent (gained in three contexts, lost in one),
which is flake, not causation. The mechanism is the D7 class: the keyboard pass
reads computed style without the settle repair, so a transitioned navbar link
can be read mid-flight. **No effect attributable to d2**, and `.is-invalid` is
applied nowhere during a Tab traversal, so C34 cannot reach this measure at all.
...
sha `c280a6e1…`) in the same worktree, the same test produced the **identical
