...
> **SHIPPED to `origin/main` 2026-06-09 — PR #57, squash `9fb1337` (feature commit `8d358b0`).** Owner-approved scope (D1–D5 locked): the second 2D slice extends the 2D-A advisory layer to the **Progression page only**. Strictly additive, advisory-only, default-off; reuses 2D-A wholesale. No new fatigue math, no suggestion-number change, no progression-decision change.
...
## Phase 2D-C Manual Adjustment Affordance

> **SHIPPED to `origin/main` 2026-06-09 — PR #58, squash `5bf4880` `feat(calibration): add manual fatigue-context nudge controls`.** The third 2D slice from the split (§"Phase 2D Recommended Implementation Split" item 3). Strictly the optional manual-adjustment affordance: the user must explicitly accept any change, it populates Workout Controls inputs only, and it persists nothing — the same client-side-only contract as the MVP `Apply suggestion`. No auto-apply. Feature branch `feat/calibration-phase-2d-c-manual-nudge` merged and deleted (local + remote); local `main` in sync with `origin/main` (0 commits ahead). CI all 8 checks green before merge.

### Scope (locked)

- **Workout Controls only.** The affordance renders inside the existing 2D-A fatigue-context section in the Workout Controls "show the math" trace.
- **Neutral manual nudge, not a fatigue-derived delta.** Each ± stepper steps the target input by its **own native step** (weight `step` attr, else 1; sets 1), clamped to the input's `min`. There is deliberately **no mapping from fatigue band/percent to a magnitude** — that is gated to 2D-D.
- **Weight + sets only; reps deferred.** A stepper renders only for a control that exists; reps (`min_rep`) is intentionally not nudged in this slice.
- **Client-side only; no API/write path.** The buttons set input values directly (no `input` event, so they do not mark the weight "user-dirty" or trigger a re-estimate); no fetch, no persistence to `user_selection` / `workout_log` / `user_profile_lifts`.
- **Reset restores the estimator suggestion exactly.** "Reset to suggestion" re-applies the last resolved estimate's `weight` + `sets` from `latestTracePayload`.
- **Reuses the shared fatigue-context toggle.** No new setting; the affordance lives inside the fatigue-context block, so the 2D-A toggle-off path (block omitted server-side) also removes the nudge controls.
- **No schema/backend behavior change.** No route, table, settings, or estimator changes.

### Files touched

- **Frontend**: `static/js/modules/workout-plan.js` (`buildFatigueNudgeControls` / `buildFatigueNudgeGroup` / `nudgeWorkoutControl` / `resolveControlStep` / `resolveControlMin` / `resetWorkoutControlsToSuggestion`, appended to `buildFatigueContextSection`); `static/css/pages-workout-plan.css` (neutral, ink-toned `.workout-estimate-fatigue-nudge*` styles, light + dark — hand-maintained route bundle, no SCSS rebuild).
- **Tests**: `e2e/fatigue-context.spec.ts` (+1 spec: nudge weight/sets independently → only the intended input changes, zero non-GET requests across the cycle, reset restores the original weight + sets; +1 assertion that the nudge affordance is absent when the toggle is off).

### Guardrails (all hold)

No suggested weight/reps/sets change *by the estimator* · no estimator-priority change · no fatigue-threshold/landmark/formula change (`utils/fatigue.py` untouched) · no plan-row (`user_selection`) writes · no `workout_log` / `user_profile_lifts` writes · no auto-apply (user clicks to nudge or reset) · reuses the shared toggle · **2D-D remains out of scope and gated.**
...
> **Re-verified 2026-08-13 — every line above still measures the same ~9 weeks later. 2D-D remains NOT STARTED.** On that evidence the owner **closed the upstream Fatigue Phase 2 Stage 4 calibration window** as "no real-use evidence, no threshold change" — measurement and reopen bar in [`docs/fatigue_meter/PHASE2_PLANNING.md`](../fatigue_meter/PHASE2_PLANNING.md) §5 Stage 4.
>
> 2D-D stays blocked, and unblock criteria 1–4 below stand **unchanged**. The one thing the close does change is the *wait*: the observer is dormant, so evidence will **not** accumulate on its own. **Reopen 2D-D only on a fresh written owner approval *plus* new evidence** — restarting real-use collection is itself an owner decision, and it must happen before criterion 2 can ever be met.
>
> A caveat on criterion 1's *historical* half: the W20 rows that produced the one real anchor are **no longer in the live database** (`workout_log` is empty, so the 21 rows logged 2026-05-20 have since been cleared). The anchor survives only as the recorded numbers in [`calibration-notes.md`](../fatigue_meter/calibration-notes.md).
...
If and only if every unblock criterion above is met, the minimum viable 2D-D to bring to the owner would be: **sets-only**, **user-accepted** (suggested, never auto-applied), **client-side only** (edits the input, same contract as 2D-C), **no persistence** to `user_selection` / `workout_log` / `user_profile_lifts`, a single coarse step (e.g. high/very-heavy band → suggest −1 set), bands consumed **read-only**, strictly post-estimate. This is a framing sketch, **not a recommendation to build** — the recommendation is to keep 2D-D parked. *(Sentence amended 2026-08-13; it previously ended "…and let the observer accumulate evidence," which the dormant observer no longer makes true — see the re-verification note above.)*
