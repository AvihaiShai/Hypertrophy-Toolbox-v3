...
| `/fatigue` visual baselines | `e2e/__screenshots__/{win32,linux}/…` (6 new each) | **N7** |
...
- **Linux: created.** Generated by the `visual-linux` deep-gate job (run `30288077299`, `run_visual=true`, `visual_mode=generate`, completed green in 15m) and copied in file-by-file from the `visual-baselines-linux` artifact.
- **V2 held exactly, on both platforms:** `git status` shows 6 *new* files per platform under `e2e/__screenshots__/` and **zero modified** existing snapshots. The copy step asserts the target does not exist before writing, so a rebaseline cannot happen by accident. N7 authorizes creation, not rebaselining.
- Matrix size **60 → 66** per platform.

#### ⚠️ Finding — 10 pre-existing Linux baselines would not reproduce

The deep-gate artifact contained all 84 regenerated baselines, so the other 60 could be compared against the committed ones without touching them. **10 differ:**

`body-composition-desktop-light` · `progression-desktop-light` · `session-summary-desktop-light` · `volume-splitter-desktop-light` · `weekly-summary-desktop-light` · `welcome-desktop-light` · `workout-log-desktop-dark` · `workout-log-desktop-light` · `workout-plan-desktop-dark` · `workout-plan-desktop-light`

Every one is a **`desktop`** variant (1440×900); no `mobile` or `tablet` baseline drifted. Differences are ~0.1% of file size.

**Nothing was overwritten** — this is pre-existing drift in the committed Linux ledger, unrelated to WP4.4, and correcting it would be a rebaseline requiring separate owner approval (V2, R3 condition 6). It is recorded here because **N8 runs the Linux deep gate at h, i, j and k**: those packets must expect up to 10 desktop-variant reds on Linux that they did not cause, and must not attribute them to their own change or "fix" them with `--update-snapshots`.
