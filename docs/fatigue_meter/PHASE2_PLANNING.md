...
**Stage 4 closed:** 2026-08-13 — owner decision, insufficient real-use evidence, **no threshold change** (see §5 Stage 4)
...
**Phase 1's** Stage 4 calibration close (2026-05-20) confirmed the §24.B thresholds are sane against one real logged week + four synthetic anchors. That removes recalibration as the next investment and frees the next investment for **resolution** (channel split + drill-down + ratio).
...
### Stage 4 — Post-merge calibration window — ✅ CLOSED 2026-08-13 (no evidence / no threshold change)
Opened 2026-05-24 for ≥2 weeks of real use before any per-muscle threshold tweaks, under the same "no tuning without ≥2 same-direction disagreements" bar as Phase 1 §4.2. Earliest close date was **2026-06-07**.

> **⛔ CLOSED 2026-08-13 — owner decision: insufficient real-use evidence, no threshold change.** **No threshold edit** — `MUSCLE_VOLUME_LANDMARKS` (`utils/_fatigue/per_muscle.py`), `SESSION_FATIGUE_BANDS` and `WEEKLY_FATIGUE_BANDS` (`utils/_fatigue/core.py`) all remain the BRAINSTORM §5 / §24.B defaults.

**Evidence re-verified read-only 2026-08-13** (live `data/database.db` snapshot, `schtasks /Query`, filesystem):

| Signal | Required to keep the window productive | Measured 2026-08-13 |
|---|---|---|
| Real `workout_log` rows | multiple distinct weeks of varied stress | **0 rows** |
| `docs/fatigue_meter/stage4_calibration_log.csv` | one pending row per observed `(period, muscle)` | **absent** — observer never had data to append |
| `HypertrophyToolbox\FatigueStage4Observer` scheduled task | installed + running daily | **not installed** |
| `logs/fatigue_stage4_latest.txt` | refreshed by each run | **last written 2026-05-30**; that run already reported `logged fatigue: 0.0`, `logged bars: (no muscles)` |
| Same-direction real-use disagreements | **≥2** (Phase 1 §4.2 bar) | **0** |

Every other user table is empty too (`user_selection` holds 1 row; `progression_goals`, `muscle_volumes`, `user_profile*`, `body_composition_snapshots`, `program_backups` all 0) — an absence of *input*, not a fatigue-engine defect.

**Reopen criteria (all three).** Stage 4 reopens only on a **fresh written owner decision** plus: (1) a non-empty, representative real `workout_log` spanning multiple distinct weeks of varied stress; and (2) **≥2 same-direction real-use disagreements** between engine band and owner felt label, recorded through the existing observer. Synthetic-generator mismatches never count (the `hard_4d` precedent — Phase 1 §4.2, `calibration-notes.md`).

**The measurement machinery is kept, not retired.** `scripts/fatigue_stage4_observer.py`, `scripts/fatigue_stage4_status.py`, `scripts/run_fatigue_stage4_observer.bat`, `scripts/install_fatigue_stage4_observer_task.ps1`, and `scripts/check_fatigue_stage4_automation.ps1` all stay on disk, as do the 26 contract tests in `tests/test_fatigue_stage4_observer.py`. What is retired is the **standing instruction to run them** — see §10.

**Downstream consumer — Learned Calibration Phase 2D-D gate (BLOCKED 2026-06-09; still blocked, re-verified 2026-08-13).** The same Stage 4 real-use evidence bar gates Learned Calibration **2D-D** ("actual suggestion modification" — letting fatigue/volume change the suggested *number*; see [`docs/user_profile/LEARNED_CALIBRATION_PLAN.md`](../user_profile/LEARNED_CALIBRATION_PLAN.md) §"Phase 2D-D Gate Review — BLOCKED (2026-06-09)"). As of 2026-06-09 the evidence is **insufficient**: live `workout_log` **0 rows**, no `stage4_calibration_log.csv` observer output, **0 of the required ≥2** same-direction real-use disagreements; the one historical real anchor (W20) agreed with the engine and the synthetic `hard_4d` mismatch does not count. 2D-D therefore stays blocked. **This note records the gate result only — no threshold/band/landmark change, no `utils/fatigue.py` edit, no `SCENARIOS` tuning.**
...
> **⛔ Observer automation is DORMANT as of the 2026-08-13 Stage 4 close (§5) — the run instructions below are retired.** Do not put the observer back on a schedule. The two bullets that follow are preserved as the **design record** of tooling that is still on disk and still tested — their "installed / verified / daily 20:00" run-state claims describe 2026-05-29/30 and are **no longer true**. If Stage 4 is ever reopened under the §5 criteria, re-read them as the restart recipe: run `scripts/check_fatigue_stage4_automation.ps1` first, then `scripts/install_fatigue_stage4_observer_task.ps1` if the owner wants the daily schedule back.

- **Stage 4 automation health check + install/repair (tooling, shipped 2026-05-30; dormant).** `scripts/check_fatigue_stage4_automation.ps1` is a strictly read-only PowerShell health check that reports — and classifies — the whole observer automation in one pass: observer `.py`/`.bat`/status-helper presence, the `HypertrophyToolbox\FatigueStage4Observer` scheduled task's enabled state / next run / last run / last result code (with plain-English meaning for the common codes, especially `0` = success and `0x800710E0` = Windows skipped/refused the run), `logs/fatigue_stage4_latest.txt` presence + mtime, the calibration CSV's pending/annotated row counts, and the live `workout_log` row count (via the read-only helper `scripts/fatigue_stage4_status.py`, which runs a single `COUNT(*)` through `DatabaseHandler`). It prints one VERDICT distinguishing **[BROKEN]** (missing files/task, disabled task, or a hard task-result failure), **[SKIPPED]** (installed but Windows refused the run — `0x800710E0`), **[IDLE]** (ran fine but `workout_log` is empty — nothing to calibrate), and **[READY]** (logged data exists, pending calibration rows available). `scripts/install_fatigue_stage4_observer_task.ps1` is the idempotent installer/repair (`schtasks /Create ... /F`, default daily 20:00, points at `scripts/run_fatigue_stage4_observer.bat`, documents the interactive-only / stop-on-battery / asleep-or-off skip limitations). All three are read-only against fatigue methodology — they never touch `MUSCLE_VOLUME_LANDMARKS` / `*_FATIGUE_BANDS` / `SCENARIOS` / boundary tests, and never write the DB or calibration CSV. Verified 2026-05-30: check reports **[IDLE]** (`workout_log` empty, last result `0`); installer re-registered the task idempotently; `schtasks /Run` succeeded (last code `0`, latest log refreshed).
- **Stage 4 calibration observer (tooling, shipped 2026-05-29; dormant).** `scripts/fatigue_stage4_observer.py` automates the *measurement* half of the Stage 4 window — it reuses `utils.fatigue_data.build_fatigue_page_context` verbatim (numbers identical to `GET /fatigue`), is strictly read-only against `data/database.db`, and never edits thresholds / scenarios / boundary tests. `--observe` (default) prints a per-period digest (planned + logged bands for all three windows) and, *only when real `workout_log` data exists*, appends one logged-side pending row per `(period, muscle)` to `docs/fatigue_meter/stage4_calibration_log.csv` (deduped: a fresh row is added only once the previous one's `felt_band` is filled). `--analyze` tallies engine-vs-felt disagreements and reports which muscles reached the **≥2 same-direction = signal** bar (Phase 1 §4.2). A per-user Windows Scheduled Task (`HypertrophyToolbox\FatigueStage4Observer`, daily 20:00, wrapper `scripts/run_fatigue_stage4_observer.bat`) runs the observer and overwrites `logs/fatigue_stage4_latest.txt` (gitignored). **This automates collection, not the methodology bar** — threshold tuning still requires ≥2 same-direction *real-use* disagreements AND a fresh owner go-ahead; synthetic data is out of scope (the `hard_4d` precedent). As of 2026-05-29 `workout_log` is empty, so the observer appends nothing and Stage 4 stays blocked on actual logged workouts.
- **Stage 4 observer/status evidence-pipeline tests (test-hardening, added 2026-06-09).** `tests/test_fatigue_stage4_observer.py` adds **26 pytest cases** locking the read-only / append-once / signal-bar contracts of the two scripts that *collect* the Stage 4 evidence gating Learned Calibration **Phase 2D-D** (`scripts/fatigue_stage4_observer.py` + `scripts/fatigue_stage4_status.py`). Coverage: `_direction` band math (engine_high / engine_low / agree / None), `_pending_combos` dedup keying, `_append_csv` header-once + parent-dir creation, `analyze`'s **≥2 same-direction = signal / 1 = noise** bar (incl. opposite-direction and agreement exclusion), and the **2D-D gate invariant** — `observe` against an **empty `workout_log` appends nothing, returns 0, and performs no DB write**; with a seeded log it appends only logged-side unfilled rows and a re-run produces no duplicate pending rows; plus `fatigue_stage4_status.main` JSON facts (0 on empty, correct count with rows, exit 0). All DB access is through the isolated tmp-path test database — the live `data/database.db` is never touched. **Tests only: no source behavior change, no fatigue thresholds / bands / landmarks, no `utils/fatigue.py` edit, no `SCENARIOS` tuning, no `data/database.db` commit.** This protects the *measurement* path, not the methodology bar — threshold tuning still requires ≥2 same-direction real-use disagreements AND a fresh owner go-ahead.
- **Phase 3 — per-muscle MEV / MAV / MRV defaults for the six unranked labels.** Stage 2 shipped BRAINSTORM §5 verbatim (12 muscles), per owner Stage 0 decision. Six canonical catalog labels still render at the bottom with neutral state and "—" for the % column: **Front-Shoulder, Rear-Shoulder, Lower Back, Hip-Adductors, Middle-Traps, Neck** (plus the `Unassigned` sentinel which is intentional and not a threshold gap). Resolution path: owner supplies vetted MEV / MAV / MRV per muscle, then either (a) extend `MUSCLE_VOLUME_LANDMARKS` in `utils/_fatigue/per_muscle.py` directly (**not** `utils/fatigue.py`, which only re-exports it), or (b) introduce the `user_fatigue_thresholds` Phase-3 calibration table per `BRAINSTORM.md §6 Option 3`. Per Stage 0 policy, do not invent thresholds without a fresh owner override.
...
- ~~Catalog `movement_pattern` cleanup (454 NULLs) — not blocking Phase 2 but worth scheduling as a separate data-quality task before any future feature that depends on movement pattern.~~ ✅ **Shipped 2026-05-25** via `scripts/fatigue_movement_pattern_cleanup.py` (+ dry-run twin). 454 NULL/blank `movement_pattern` rows → 0: **76 inferred** via `utils.movement_patterns.classify_exercise()` (longest-keyword-first name match, primary_muscle fallback) — hinge 31, upper_isolation 14, vertical_pull 7, horizontal_pull 6, core_dynamic 6, horizontal_push 5, squat 4, lower_isolation 3; **378 → `"unassigned"` sentinel** (lowercase, matches existing `movement_pattern` enum-style convention; distinct from Stage 1's title-cased `"Unassigned"` muscle sentinel). Idempotent (second run touches zero rows). New invariant `tests/test_catalog_invariants.py::test_catalog_movement_pattern_has_no_nulls` added and green. **pytest 1442 → 1443 passed** (~3m 02s). Fatigue math unchanged — `utils/fatigue.py::_resolve_pattern_weight` treats unknown patterns identically to NULL (both fall through to `DEFAULT_PATTERN_WEIGHT`), and none of the 454 affected rows currently appear in `user_selection` or `workout_log` so the warning-log path does not fire in practice. Pre-flight backup: local id 5, label `pre-movement-pattern-cleanup-2026-05-25`.
...
- **The one code-side guardrail — FIXED 2026-08-14.** [`scripts/fatigue_stage4_observer.py::analyze`](../../scripts/fatigue_stage4_observer.py) printed, on the `--analyze` signal path, *"… before editing utils/fatigue.py landmarks/bands."* That sentence was live and prescriptive and located **both** landmarks and bands at the re-export facade — the same imprecision the rest of this pass fixed in the docs, but higher-consequence, because it prints at exactly the moment an agent is told tuning is permitted. It now names the leaves, printing *"go-ahead before editing utils/_fatigue/per_muscle.py landmarks / utils/_fatigue/core.py bands."* — but that is the **assembled** sentence: the source splits it across two adjacent string literals, so grepping it whole returns nothing. To find the site, grep `utils/_fatigue/per_muscle.py landmarks`, which is a single source line inside `analyze()`. Owner widened the documentation-only scope by this single line. It was the only such site — a sweep of `scripts/`, `utils/` and `tests/` found every other `utils/fatigue.py` mention to be an import or a provenance docstring, and no test pinned the printed text.
- **Retention assessment of the committed `docs/fatigue_meter/` artifacts (measured 2026-08-13; outcome: keep both, delete nothing).** The Stage 4 close was authorized to retire generated artifacts *proven regenerable and unpinned*. Both artifact families pass the pin test and **fail** the regenerable test, so both stay:
  - **`generated-calibration-report.md` — keep.** No test or workflow reads it (the only code reference is `scripts/fatigue_calibration_report.py`, which *writes* it as `DEFAULT_OUTPUT`). But regenerating it today is **not** reproduction: a fresh `--seed 42` run against the current catalog reproduces every calibration-relevant number exactly (weekly 28.1 / 88.5 / 161.9 / 419.6, all session scores, set counts, bands) while **89 of 249 lines still differ**, including **74 per-exercise routine rows** and every routine heading — the catalog and the generator's routine naming both moved after the 2026-05-11 run. Those per-exercise tables are the "full routine data" the report exists to show, and they are what the owner read when supplying the four felt labels behind the Phase 1 Stage 4 close (`calibration-notes.md` 2026-05-20), so the v23 leftovers premise *"regenerable script output, so deleting it loses no evidence"* is **disproven**.
  - **`baseline-2026-04-30.txt` / `baseline-2026-04-30-v2.txt` — keep.** Not script output at all: they are one-shot Stage 1.1 test-run captures pinned to HEAD `818e881` with a stash ref, and nothing can regenerate them. `PLANNING.md` §1.1 cites them three times (`:76-78`) and explicitly preserves v1 as the red-state record; the document cites them six times in all (`:10`, `:76-78`, `:139`, `:410`).
...
| [`calibration-notes.md`](calibration-notes.md) | **Phase 1** Stage 4 close 2026-05-20 — why thresholds are stable enough to invest in resolution next. |
| [`STAGE4_PARKED_HANDOFF.md`](STAGE4_PARKED_HANDOFF.md) | Superseded by the 2026-05-20 owner-approved **Phase 1** Stage 4 close; preserved for history. |
| [`../LEFTOVERS_BY_PRIORITY.md`](../LEFTOVERS_BY_PRIORITY.md) **P2.1** | Retired 2026-08-13 with this Stage 4 close; kept as the decision record. |

---

*End of PHASE2_PLANNING.md. Stage 0 + Stage 1 closed 2026-05-23. Stage 2 implementation completed 2026-05-23 on branch `feat/fatigue-meter-phase-2-stage-2`, merged to `main` 2026-05-23 via PR #35 (squash commit `d5b80bf`). Stage 3 verify-suite gate closed 2026-05-24 on `main` (pytest 1442 passed; Playwright Chromium 449 passed / 13 failed / 17 did-not-run — reds match documented pre-existing baseline exactly, zero new Stage-2 reds). Stage 4 calibration window opened 2026-05-24 and **closed 2026-08-13 by owner decision — no real-use evidence, no threshold change** (see §5 Stage 4). Path 1 ships dedicated `/fatigue` route, dual planned + logged per-muscle bars sorted by % MRV, two SFR cards with `fatigue == 0 → "—"` sentinel, period selector (`this session` / `this week` / `last 4 weeks`), nav link in Analyze dropdown, badge "View per-muscle breakdown →" link. Phase 1 surface preserved byte-identical. Phase-3 follow-up: vetted MEV / MAV / MRV defaults for the six unranked labels (Front-Shoulder, Rear-Shoulder, Lower Back, Hip-Adductors, Middle-Traps, Neck) — see §10.*
