...
- **The one code-side guardrail — FIXED 2026-08-14.** [`scripts/fatigue_stage4_observer.py:265`](../../scripts/fatigue_stage4_observer.py) printed, on the `--analyze` signal path, *"… before editing utils/fatigue.py landmarks/bands."* That sentence was live and prescriptive and located **both** landmarks and bands at the re-export facade — the same imprecision the rest of this pass fixed in the docs, but higher-consequence, because it prints at exactly the moment an agent is told tuning is permitted. It now names the leaves: *"… before editing utils/_fatigue/per_muscle.py landmarks / utils/_fatigue/core.py bands."* Owner widened the documentation-only scope by this single line. It was the only such site — a sweep of `scripts/`, `utils/` and `tests/` found every other `utils/fatigue.py` mention to be an import or a provenance docstring, and no test pinned the printed text.
