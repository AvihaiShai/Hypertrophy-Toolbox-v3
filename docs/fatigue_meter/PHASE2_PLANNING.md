...
- **Open follow-up (recorded 2026-08-14, not actioned): one guardrail still lives in code and still names the facade.** [`scripts/fatigue_stage4_observer.py:263-265`](../../scripts/fatigue_stage4_observer.py) prints, on the `--analyze` signal path, *"Threshold tuning is \*eligible\* but still requires a fresh owner go-ahead before editing utils/fatigue.py landmarks/bands."* That sentence is live and prescriptive, and it locates **both** landmarks and bands at the re-export facade — the same imprecision the 2026-08-14 docs pass fixed everywhere else. It matters more than the doc copies did, because it prints at exactly the moment an agent is told tuning is permitted. The correct text names the leaves: `utils/_fatigue/per_muscle.py` landmarks / `utils/_fatigue/core.py` bands. **Not fixed here** — that pass was scoped documentation-only and this is a `.py` edit. It needs an owner decision to widen scope, and it is the only such site: a sweep of `scripts/`, `utils/` and `tests/` found every other `utils/fatigue.py` mention to be an import or a provenance docstring.
