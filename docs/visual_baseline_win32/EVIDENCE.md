...
> — silently drops the entire `stylelintSevenSurfaces` block. It changed **171 leaf values to fix
...
`artifacts/review/{win32-visual-OLD,win32-thumbs-OLD}`, built by the same tool from the 79 baselines
as committed at `HEAD` — read out of the git object store, so no working copy was involved. 52
