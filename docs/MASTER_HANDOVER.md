...
| app.py review | **APPROVED for execution 2026-08-01** (was DRAFT; plan committed via PR #218). Started as a cross-check of an external review of `app.py` whose content was lost in transit; the owner then directed an independent review. Findings F1–F3 were confirmed **empirically** (real app object, `DB_FILE` on a scratch database, requests through `test_client()` with `DEBUG=False, TESTING=False`), independently reproduced by a Codex round that raised six corrections, and re-checked a third time at source level. A third-round `internal_error` candidate was tested and **dismissed** (§3c). All four owner decisions are signed (§5 D1–D4): delete `clear_trailing`; introduce `utils/version.py` as the cache-buster source; approve the 418→500 test flip; run all five packets now. **The finding surface is exhausted — do not commission another review round.** | Execute §6: P1 → P5, with P2 and P3 alongside, P4 last. P1 and P4 need `/verify-suite` and must serialize against each other (`playwright.config.ts:67` pins port 5000). P1's PR needs migration notes — it changes response status codes on currently-wrong paths. | [docs/APP_PY_REVIEW_PLAN.md](APP_PY_REVIEW_PLAN.md) |
...
- **The app.py review is APPROVED and executing** (2026-08-01). All four owner
  decisions signed; see [`APP_PY_REVIEW_PLAN.md`](APP_PY_REVIEW_PLAN.md) §5–§6.
