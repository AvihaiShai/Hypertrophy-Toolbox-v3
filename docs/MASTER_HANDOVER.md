...
>
> **Windows has a CI path to *measure* the corpus, and still none to produce it.**
> `deep-gate.yml` now carries a second visual job, `visual-windows`, pinned to
> `windows-2022` and gated behind the `run_visual_windows` input. It is **compare-only by
> construction** — no generate mode, `--update-snapshots` never passed, and a final step
> fails the job if anything under `e2e/__screenshots__` changed — so
> `e2e/__screenshots__/win32` is still regenerated **locally or not at all**.
>
> **Read a red from that job carefully.** The Linux corpus was generated by the same
> workflow on the same pinned image, so a `visual-linux` red means the code moved. The
> win32 corpus was generated on the owner's **local** machine (#309), so a `visual-windows`
> red may mean only that runner rendering differs from local rendering — different Chromium
> build, different installed font set, DirectWrite. Until that delta is measured, treat the
> job as an instrument, not a verdict.
>
> **It is not a required check and cannot become one by accident** (`workflow_dispatch`-only,
> and an `if:`-gated job cannot block merge). Promoting it would first require regenerating
> the corpus **on the runner image** and reviewing it — a separate owner decision, and the
> point at which the local corpus stops matching.
>
