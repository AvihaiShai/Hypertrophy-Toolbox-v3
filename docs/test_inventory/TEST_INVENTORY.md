...
| pytest collected nodes (deterministic subset) | **3192** across 131 files |
| pytest test files (all) | **132** |
...
| File | Collected |
|---|---:|
| `tests/test_agent_workflow_contracts.py` | 94 |
...
| `tests/test_effective_sets.py` | 63 |
...
| `tests/test_export_blank_rep_bounds_contract.py` | 5 |
| `tests/test_export_blank_rep_bounds_gate0.py` | 12 |
| `tests/test_export_weekly_summary_sheet.py` | 4 |
| `tests/test_exports.py` | 57 |
...
| `tests/test_fatigue_routes.py` | 17 |
...
| `tests/test_rep_range_integrity.py` | 50 |
...
| `tests/test_worktree_ci_contracts.py` | 12 |
| `tests/test_worktree_environment_preflight.py` | 72 |
| `tests/test_worktree_lifecycle.py` | 146 |
| `tests/test_worktree_snapshot.py` | 33 |
| `tests/test_youtube_video_id.py` | 40 |

## Vitest test files

Collected with `vitest list --json=<path>`, which lists cases without running them. `.skip`, `.todo` and `describe.skip` cases are omitted by the lister entirely, so a disabled case is indistinguishable here from a deleted one and **no status is recorded**: a count of 245 means 245 cases that would run.
