...
total moves **2857 → 2869**. File counts are unchanged at **125** deterministic
...
**Both totals were re-derived after the rebase onto `8c844df`
([#454](https://github.com/AvihaiShai/Hypertrophy-Toolbox-v3/pull/454)), not carried
forward.** Against the original `116d3c5` base this packet read **2855 → 2867**; #454
added two nodes to `tests/test_visual_capture_contracts.py` (15 → 17), moving the base
to **2857**. The **51 → 63** figure is unchanged because #454 did not touch this file,
and `test_visual_capture_contracts.py` stands at **17** in the merged tree — #454's
count, preserved. The inventory was regenerated by
