...
| # | Residual | Why accepted |
|---|---|---|
| **R-10** | The converted job has **never executed as a `uses:` job**. `deep-gate.yml` runs only on its schedule and on `workflow_dispatch`, and `workflow_dispatch` requires the file to be on the default branch — so, exactly as with `release.yml` under A-B1, there is **zero runtime evidence available before merge**. Everything above is static. | The same sequencing R1 accepted, with the same compensation: the first post-merge action is a `workflow_dispatch` of `deep-gate.yml` whose `frozen-windows` result is inspected. R2-b is not complete at merge. **This dispatch is not the held inspection and does not discharge the hold** — with default inputs it skips `visual-linux`, so it validates the conversion and says nothing about the weekly gate. The two obligations are separate; the hold is checklist rows 1–3 above. **DISCHARGED 2026-08-16** — `workflow_dispatch` [31972476567](https://github.com/AvihaiShai/Hypertrophy-Toolbox-v3/actions/runs/31972476567) on `main` @ **`949b15e`** (the merge commit itself), 21:05:54Z → 21:22:05Z: **7/7 jobs success**. `frozen-windows` executed **as a `uses:` job for the first time**, reported under the composite name `Frozen executable (real bootloader, Windows) / Build and smoke`, and ran Stage tracked package assets → Build via the canonical spec → Smoke the real bootloader. One correction to this row's own expectation: it was dispatched with `run_visual=true visual_mode=compare`, **not** default inputs, so `visual-linux` also executed and compared without writing a baseline. **Still a dispatch: it says nothing whatever about the `schedule` trigger, and does not touch the waived hold.** |
