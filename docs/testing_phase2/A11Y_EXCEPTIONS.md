...
- **[Axe register](#axe-register)** (X7–X16; X11, X12 and X13 are resolved) — WCAG violations the shipped app produces today.
...
| **X12** | `/volume_splitter` `.form-range.volume-slider[data-muscle]` (all 18) | WCAG 4.1.2 Name, Role, Value | Every muscle slider is unlabelled. The muscle name is rendered as adjacent text, never associated. The page is almost entirely a form, so this is the single largest functional loss in the app for a screen-reader user. | was `label` ×18 on `volume_splitter:{light,dark}`; both rows are now `color-contrast` only | **RESOLVED — shipped in #393.** `createSliderRow()` gives the `.muscle-name` span an id and points the range input's `aria-labelledby` at it. Deliberately **not** a `for=` on the wrapping `<label>`: that label also contains the live `.current-value` pill, so the association would name the slider "Neck 12" and mutate the name on every drag. Ids are keyed on the render index, which `renderSliders()` makes unique by construction. **Advanced mode was verified separately:** it renders 32 sliders through the same `createSliderRow()`, and an A/B there took `label` ×32 to zero. Measured via `?mode=advanced` with no interaction, its default paint is `color-contrast` ×2 — **identical to basic**, so advanced mode contributes no violation of its own and the register loses nothing by scanning the default paint. What is genuinely unscanned is the *post-Calculate* state, recorded as **X16**. |
| **X13** | `/workout_log` `.btn-group` | WCAG 4.1.2 Name, Role, Value | A `div` carries `aria-label` with no `role` to hang it on, so the name is discarded. | was `aria-prohibited-attr` ×1 on `workout_log:{light,dark}`; both rows are now `color-contrast` only | **RESOLVED — shipped in #393, by removing the name rather than by adding a role.** Three fixes clear this node identically and all were measured: deleting the element, `role="group"`, and dropping the `aria-label`. **Deletion was preferred on review and rejected by measurement** — the element is empty (`childElementCount` 0, a `0×0` rect, referenced by no JS, CSS or test), but as `display: inline-flex` it still generates a line box, so removing it shortened the page `1440×1095` → `1440×1074` and failed all six win32 `workout-log-*` captures on a size mismatch, which no pixel threshold can absorb. **A 0×0 `getBoundingClientRect()` is not evidence that a node contributes no layout.** Between the two remaining options the name was dropped, because `role="group"` would announce a named group of export controls that do not exist: `GET /export_workout_log` is implemented and tested but bound to no UI at all ([`docs/product/APP_FLOW.md`](../product/APP_FLOW.md) §Known discrepancies). **The honest fix is the missing export button, which is a product gap, not an accessibility one** — binding it makes the group real and non-empty, and would carry the 12-capture re-baseline that deletion also needs. |
...
| **X16** | `/volume_splitter` `div[aria-label="Volume distribution table"]` — the results-table wrapper, **after Calculate**, in **both** modes | WCAG 2.1.1 Keyboard | The results table scrolls (`.volume-insights-panel .tbl-wrap` takes `overflow: auto` and a `max-height` in `pages-volume-splitter.css`) but contains no focusable descendant, so a sighted keyboard-only user cannot scroll it. The wrapper already has `role="region"` and a name, which gives landmark navigation but **not** keyboard scrolling. Not volume-splitter-specific: `layout.css` gives every `.tbl-wrap` `overflow-x: auto`, and all 13 template instances carry `role="region"` + `aria-label` with no `tabindex` — this is the one that happens to overflow. | **Not registered** — this row records the boundary. The route scan is a bare `goto`, and `.results-section` ships `d-none` until `calculateVolume()` runs, so no registered surface reaches this state in either mode. | **Found while fixing X12, and initially mis-attributed by me to advanced mode.** A four-way control settled it: basic default and advanced default both read `color-contrast` ×2 and nothing else, while basic+Calculate reads `color-contrast` ×3 + `scrollable-region-focusable` ×1 and advanced+Calculate reads ×4 + ×1. **The axis is Calculate, not mode.** Fix is one pixel-neutral attribute (`tabindex="0"` on a wrapper that already has role and name), but done honestly it is a 13-site change adding a tab stop to every table page, and it would ship unasserted because no registered surface reports the rule. Fix and coverage belong in one owner-gated packet, the way X1 was done. |
...
6. Every rule id tolerated by `AXE_REGISTER` must appear here as a code span.
   `tests/test_axe_contracts.py` enforces that in the required pytest gate. **A rule id whose only
   code span sits in a row marked RESOLVED does not satisfy this rule for a *new* registration.**
   Rule 3 keeps resolved rows forever, so `select-name`, `label` and `aria-prohibited-attr` now have
   permanent code spans here while appearing nowhere in the register. If one is ever registered
   again on some other surface, that gate would pass on the strength of a closed row — write the
   new row first. Detection is unaffected either way: the spec compares the *whole* violation list
   per surface, so an unregistered rule reds regardless of what this file says.
