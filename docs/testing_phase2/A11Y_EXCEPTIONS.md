...
- **[Axe register](#axe-register)** (X7–X15) — WCAG violations the shipped app produces today.
...
Every row below is a real defect. **None is fixed here, by owner decision of 2026-08-14:** Packet D
takes the explicit-exception path — axe still executes every WCAG rule, and each existing violation
is pinned by surface, rule id and exact node count rather than suppressed. The colour/token rewrite
and the two-platform visual re-baseline are ruled out for this arc, and the production corrections
for X7–X13 and X15 are **owner-deferred**, not declined and not blocking.
...
**Measurement note — dark counts are post-#365.** The first draft of this register was measured
before Packet F restored the theme-switch transition suppression. `setThemeAndSettle` samples
`body` alone, and `body` reached its final colour while slower elements were still cross-fading, so
axe was scoring partly-transitioned colour on dark surfaces. Two rows were understated:
`volume_splitter:dark` (2 → **3**) and `fatigue:dark` (4 → **6**). Proven by reverting #365, which
puts both back to the old numbers, and by three consecutive repeats at each state showing zero
drift. Every dark count here is therefore a settled-colour reading, and re-measuring the register on
a tree without #365 will disagree with it by design.
...
| **X8** | `/fatigue` dark: `h1`, `.fatigue-page__subtitle`, `label`, `#fatigue-period`, `p[data-testid="fatigue-empty-state"]` and its `a[href$="workout_plan"]` | WCAG 1.4.3 Contrast (Minimum) | **Six** nodes. Recorded separately from X7 because `templates/fatigue.html` links no page bundle at all, so this route is painted entirely by the shared global bundles — these six are a direct reading of shared CSS, not of page-local overrides. `/fatigue` **light is clean**, which makes the pair the app's sharpest signal on whether a shared-token change helped or hurt. An earlier draft of this row recorded **four** nodes with ratios 3.97:1 / 2.49:1 / 3.97:1 / 4.13:1; that reading predates #365 and understated the surface — see the measurement note above. `h1` and `#fatigue-period` are the two it was missing. | `color-contrast` ×6 on `fatigue:dark`; `fatigue:light` registered as an explicit empty list | **OWNER-DEFERRED 2026-08-14.** Belongs to the same future token packet as X7; keep the row distinct so the shared-bundle reading is not averaged away. |
...
| **X11** | `/progression` `#exerciseSelect` | WCAG 4.1.2 Name, Role, Value | The control the entire page is driven by has no `<label>`, `aria-label` or `aria-labelledby`. Its visible heading ("Select Exercise to Progress") is an `<h4>` with no programmatic association. | `select-name` ×1 on `progression:{light,dark}` | **OWNER-DEFERRED 2026-08-14.** Smallest fix of the set — one `aria-labelledby` — but still a production change, and this arc is test/docs-only. |
| **X12** | `/volume_splitter` `.form-range.volume-slider[data-muscle]` (all 18) | WCAG 4.1.2 Name, Role, Value | Every muscle slider is unlabelled. The muscle name is rendered as adjacent text, never associated. The page is almost entirely a form, so this is the single largest functional loss in the app for a screen-reader user. | `label` ×18 on `volume_splitter:{light,dark}` | **OWNER-DEFERRED 2026-08-14.** The largest functional loss of the set, and still a production change — slider labels are explicitly excluded from this arc. |
| **X13** | `/workout_log` `.btn-group` | WCAG 4.1.2 Name, Role, Value | A `div` carries `aria-label` with no `role` to hang it on, so the name is discarded. | `aria-prohibited-attr` ×1 on `workout_log:{light,dark}` | **OWNER-DEFERRED 2026-08-14.** Real defect, pinned, fix out of scope for this arc. |
...
2. A row marked **Owner decision needed** must not be fixed as a side effect of another packet; it
   ...
   measured and pinned, whose production fixes are explicitly out of scope for this arc.
...
5. X7–X13 and X15 are pinned at exact node counts. If a count moves, the fix is to find out *why* —
