...
| KI-004 | Toast (`templates/base.html:236`, `static/js/modules/toast.js:314-333`) | Single `#liveToast` element re-used on rapid successive `showToast()` calls — last message wins, prior message can be lost mid-fade. | 🟡 Medium | Mitigated | Module disposes the prior Bootstrap instance to prevent animation conflicts. `e2e/ui-hardening.spec.ts` asserts: only one toast element exists, last message wins, stale `bg-*` classes are cleared. |
...
| KI-010 | Toast legacy signature (`static/js/modules/toast.js:232-234`, applied at `:236`) | **Was**: a legacy call whose *message* is itself one of the four type words was swallowed. `showToast('error', true)` rendered body text **`"true"`** on a `bg-danger` toast, because `validTypes.has(type)` saw a valid type and the legacy branch never ran. Identical for `'warning'`, `'success'` and `'info'`. The **one-argument** form collided the other way: `showToast('warning')` rendered the default copy **"Action completed successfully."** on a yellow toast. | 🟡 Medium | Mitigated | **Repaired 2026-08-27 (Packet U3a).** The legacy/modern discriminator no longer reads argument 1 alone: argument 1 is a **type** only when it is one of the four words **and** argument 2 is not a boolean **and** argument 2 was actually supplied (`arguments.length < 2`). **Twelve defective outcomes** are fixed — all four type words in all three arities. The packet's signed Section 0 enumerates **nine** of them, because it lists the `false` arity for `'error'` only (`"false"` on **red** when the caller asked for green); the defect was in fact present for all four type words in that arity, which is why **B46 ships parametrised over four**. **Nine is a known undercount and is not restated here as a current fact** — the signed figure is preserved in place and the discrepancy is recorded at `PLANNING.md` §i.1/§i.2. **Dispatcher-only by owner ruling OD-5: no call site was migrated and the legacy signature still ships.** Re-measured at fix time: **13** collision-capable call sites (**8** two-argument, **5** one-argument), of which **11 are user-visible** — `workout-plan.js:114` sits behind an `if (!error.code)` guard that `normalizeError` makes always-false, and `filters.js:251` is clobbered in the same tick by `:255` under KI-004. Server-side reachability re-measured: **0** of **229** `success_response()` / `error_response()` sites pass a bare type word, so the defect was latent, not live. **Locking tests, all in `static/js/modules/__tests__/toast.test.js`:** **B45a–d** (`showToast(T, true)` renders `T` on `bg-danger`) and **B43a–d** (`showToast(T)` renders `T` on `bg-success`) are the **deliberately inverted** characterization cases — the red a reviewer sees without the production fix is the intended signal. **B46a–d** pins the severity inversion and, **with B47a–d**, is what kills the narrowing mutation `message === true` — arm 5c reds the two families together and nothing else. **B47a–d** pins a **new** consequence: `showToast(T, false, {requestId})` now **drops** the request-ID suffix, because the legacy path resolves the type to `success` and the suffix gate is error-only. **B13 is unchanged and load-bearing** — an explicitly supplied `undefined` stays a *modern* call, so a modern error toast is never repainted green. This restores fidelity, not quality: a toast whose entire body is the word `"success"` on a red background is less wrong than `"true"`, not good copy, and severity still comes from the boolean and never from the message. `new Boolean(true)`, `1` and `0` remain **uncovered** — `typeof` sees them as non-boolean and CI-6/B8 keeps them contract-legal. Plan and owner rulings: [`docs/toast_type_word_collision/PLANNING.md`](toast_type_word_collision/PLANNING.md). | ⚠️ **RE-ANCHORED 2026-08-27 on integration.** The citation above read `:50-66`, measured against the **pre-KI-011** file. **KI-011 shipped first** (PR #426, `5b35966`) and rewrote the rendering half of this module, so every line number in it moved. The anchor now names the `isLegacyCall` predicate as it sits on merged `main` plus this change, measured rather than derived from a drift figure. **The two fixes occupy different halves of `showToast()`** — KI-010 decides how the ARGUMENTS are read, KI-011 decides how the toast RENDERS — and both are live.
| KI-011 | Toast action button (`static/js/modules/toast.js:60` + `:84`; callers `static/js/modules/volume-splitter.js:424-431` and `:184-190`) | Any **later** `showToast()` from anywhere destroys a still-live action button: `:60` clears `toastBody.innerHTML` and `:84` appends the button into that same `#toast-body`. **Reachable inside a caller of its own** — the "Activate for Plan tab" toast is raised with `duration: 6000`, `volume-splitter.js:433` immediately calls `loadVolumeHistory()`, and the `.catch` opening at that function’s `:554` emits `showToast('error', …)` at `:564`. On a slow or failing history refresh the user’s button vanishes mid-toast. ⚠️ **AMENDED 2026-08-26 (Packet U1).** This row said **"sole caller … `:299-306`"**, and the KI-012 row below falsifies the "sole" half: U1's Retry action is the **second** caller, at `:184-190`. The four `volume-splitter.js` anchors above were all exact when written and all moved **+125** lines under U1's diff; they are re-anchored here, measured against the shipped file rather than derived from a single drift figure. `toast.js`'s own two anchors did not move — U1 does not touch that file. | 🟡 Medium | Mitigated | **Repaired 2026-08-27 (Packet U3b).** `showToast()` no longer clears `#toast-body` wholesale: it replaces only `span.toast-message` and keeps the action in a `div.toast-action-slot.d-inline` beside it, **inside `#toast-body`** so `expectToast` and the `Plan #N saved. / Activate for Plan tab` contract at [`volume-splitter.spec.ts`](../e2e/volume-splitter.spec.ts) still read it. An action lives for **its own raising toast's duration**, on its own timer, and is invalidated by activation, dismissal or expiry; a later toast with no action **preserves** it, one with an action **replaces** it, and a standing action **extends** the toast to the later deadline. **`toast.test.js` is byte-identical at 47 cases** — the regression is **E2E-only by owner ruling**, so the JS-unit qualification window's **T0 is preserved**. Locking tests, all in [`e2e/volume-splitter.spec.ts`](../e2e/volume-splitter.spec.ts): **`t1`** (the action survives an unrelated later message), **`t2`** (the surviving button still fires its **original** closure — a re-rendered button would keep the node and lose the id), and **`t8`** (focus on the action survives a message replacement, which nothing at any tier asserted before). `t3`–`t7`, `t5b` and `t12` lock the replacement, duration, expiry, inertness, dismissal and placement rules; `t9`–`t11` lock the U1 amendments below. **Two further defects were found while fixing this one and are fixed here too:** a **deterministic uncaught `TypeError`** from Bootstrap whenever a replacement landed inside the ~150 ms show transition (measured 2 errors at gap 0, clean from 100 ms up; `t12` is its oracle, and the shared `consoleErrors` fixture is **blind** to it because it filters both `classList` and `Cannot read properties of null`), and **silent focus loss** when the button was destroyed under the user's focus. Plan, owner rulings and the mutation record: [`docs/toast_action_continuity/PLANNING.md`](toast_action_continuity/PLANNING.md). `toast.test.js` **B30–B35 are deliberately placement-neutral**: they locate the button through `#liveToast` and assert its type, label, `aria-label`, guard and coercion while staying **silent about its direct parent**, precisely so the relocation a fix requires does not red them (owner ruling, [`STEP12_JS_UNIT_GATE0.md`](testing_phase3/STEP12_JS_UNIT_GATE0.md) §10.11 ruling 4). They therefore neither fix nor mitigate this defect. **No fixed-behavior regression test exists yet** — one must be added with the fix, and all six of B30–B35 must stay green through it. Measured evidence: §10.7-R10 of the same document. |
| KI-012 | Volume Splitter calculate (`static/js/modules/volume-splitter.js`, the `POST /api/calculate_volume` call) | **Was**: a failed calculation produced no user-visible signal of any kind. Two independent suppressions sat on one call — `showErrorToast: false` in the request options silenced the shared wrapper's toast for the **request-failure** class (non-2xx and transport), and a `.catch` whose entire body was `console.error` silenced the **post-2xx response-handling** class, which never reaches the wrapper's error branch at all. Reproduce on `main` before this change: calculate successfully, force `POST /api/calculate_volume` to 500, calculate again — the previous run's table, suggestion cards, `.muscle-row` status classes and `.current-value` pill modifiers all stay on screen with nothing to say they are stale. | 🟠 High | Mitigated | **Repaired 2026-08-26 (Packet U1).** A failure now raises an accessible toast carrying a Retry action **and** a persistent inline `#volume-calculate-error` region prepended to `.volume-insights-panel`, and `clearResults()` empties every surface that could be mistaken for the failed calculation's output. The region exists **if and only if** the last completed calculation failed and the user has not since reset — it is created on failure, `.remove()`d on the next success, and never present as a hidden shell, so the success path gains no observable state. Repeat **slider-originated** failures do not re-announce while the same region and U1's own toast **message** stand; explicit commands always announce. ⚠️ **AMENDED 2026-08-27 (Packet U3b).** This sentence read *"toast content"* until KI-011 was fixed. The two were co-extensive while `toast.js:60` destroyed message and button together; they now diverge in **both** directions — a replacement leaves the button but takes the message, and an auto-hide takes the button but leaves the message. OD-2 was **re-signed** on the message reading (`toast_action_continuity/PLANNING.md` §4.3), and `t9`/`t11` lock both directions. Locking tests, all in `e2e/volume-splitter.spec.ts`: **`a1`** (non-2xx after a prior success) and **`a2`** (transport abort) lock the request-failure class; **`b1`** locks the post-2xx class through a 200 the response handler cannot render. Mutations **M1** and **M2** prove the two classes fail *in isolation* — restoring either suppression alone reds only its own arms. **Not fixed here**: `showErrorToast: false` stays in production so the page-specific toast can carry Retry and one failure does not raise two competing notifications; the owner explicitly accepted at Gate 1 that the flag therefore ships without direct regression pressure (`docs/volume_failure_feedback/PLANNING.md` §v2.13, OD-4). U1 is also KI-011's **second** caller of the toast action button and does not fix that defect — the durable Retry is the inline region. Plan and owner decisions: [`docs/volume_failure_feedback/PLANNING.md`](volume_failure_feedback/PLANNING.md). |
...
|----------|------|--------|
| Multiple toasts at once | 🟡 Message loss risk | ✅ Mitigated for **message** loss (KI-004) — `e2e/ui-hardening.spec.ts` asserts single `#liveToast` instance, last message wins, stale `bg-*` classes cleared. The **action button** that same shared element used to destroy is **mitigated too** as of 2026-08-27 — see **KI-011** |
| Long error message overflow | 🟡 Truncation risk | Open — visual concern only |
| Legacy vs new `showToast` signatures | 🟡 Inconsistent style risk | ✅ **Mitigated 2026-08-27 (Packet U3a).** Both signatures are **still supported by design** (owner ruling OD-5 — dispatcher-only, no caller migrated); the discriminator no longer reads argument 1 alone, so a message that equals a type word is rendered rather than swallowed. **See the KI-010 row in §0**, which owns the contract |
