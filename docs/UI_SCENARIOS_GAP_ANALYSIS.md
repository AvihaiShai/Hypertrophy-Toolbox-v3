...
| KI-004 | Toast (`templates/base.html:236`, `static/js/modules/toast.js:127-148`) | Single `#liveToast` element re-used on rapid successive `showToast()` calls — last message wins, prior message can be lost mid-fade. | 🟡 Medium | Mitigated | Module disposes the prior Bootstrap instance to prevent animation conflicts. `e2e/ui-hardening.spec.ts` asserts: only one toast element exists, last message wins, stale `bg-*` classes are cleared. |
