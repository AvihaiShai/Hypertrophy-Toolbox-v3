...
| Legacy vs new `showToast` signatures | 🟡 Inconsistent style risk | ✅ **Mitigated 2026-08-27 (Packet U3a).** Both signatures are **still supported by design** (owner ruling OD-5 — dispatcher-only, no caller migrated), but the discriminator no longer reads argument 1 alone: `toast.js:55-72` treats argument 1 as a **type** only when it is one of the four words **and** argument 2 is a **supplied non-boolean**. A message that equals a type word is therefore rendered, not swallowed — see the **KI-010** row in §0 |
