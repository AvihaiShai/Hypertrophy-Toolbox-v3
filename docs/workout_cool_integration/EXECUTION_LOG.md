...
| 18 committed baseline PNGs — `plan-{desktop,tablet,mobile}-{light,dark}-{simple,advanced}.png` (12) + `log-{desktop,tablet,mobile}-{light,dark}.png` (6). *(Historical 2026-05-23 count; the corpus was later split by platform and capture exemptions changed the current per-platform count.)* | [`e2e/__screenshots__/win32/visual-baseline-thumbnails.spec.ts-snapshots/`](../../e2e/__screenshots__/win32/visual-baseline-thumbnails.spec.ts-snapshots/) |
