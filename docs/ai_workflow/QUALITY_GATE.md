...
**Required reviewers.** The table's Required reviewers column is the only source of diff-time reviewer requirements. Combine tests and reviewers across applicable rows, respecting the table's specificity rules. A full-suite or full-gate requirement adds tests, not reviewers. Run `unslop-reviewer` only when the user explicitly requests it; routine style cleanup is the implementing agent's responsibility. Reviewers stay independent of the implementer ([AUTONOMY.md](AUTONOMY.md#workflow-roles)).
...
- **`/unslop`** — routine post-implementation gate. Targeted tests + the reviewers the change-type table requires.
- **`/verify-and-polish`** — full gate before milestones / refactors / schema changes. `/verify-suite` (full pytest + Chromium E2E) + the reviewers the change-type table requires.
