...
/* Capture controls every platform shares: one compositor path, one completed
   frame, one colour profile. Locked by tests/test_visual_capture_contracts.py. */
...
/* The two remaining Chromium raster controls, applied on Linux only.
   `--disable-lcd-text` pins one text-antialiasing path: without it the ubuntu
   runner rendered the same glyphs through LCD subpixel AA in one attempt and
   grayscale AA in the next, at identical layout and data.
   `--disable-partial-raster` stops the renderer re-rastering only an
   invalidated sub-rect over previously rastered tile content, which is what
   made a single table row's rounded borders and box-shadows differ while the
   rest of the page matched byte-for-byte.

   Linux-only on purpose. The two baseline sets are maintained independently
   (e2e/__screenshots__/{linux,win32}), only the Linux set is generated by CI,
   and only Linux showed the defect - a 20x repeat of both workout-table specs
   on win32 passed 40/40 against the committed win32 raster. Enabling these
   there would restate every committed win32 PNG for no measured gain. */
